...
import type { DigestResponse, HttpTransport, HttpTransportRequest } from "../../vapix/digest";
import { DigestTransportError } from "../../vapix/digest";
import { sanitizedRequestTarget } from "../../../demo/witness/hygiene";

interface MediatorBackend {
  exchange(request: Pick<HttpTransportRequest, "method" | "url" | "body">): Promise<DigestResponse>;
}

// Witness-logging in-memory transport for the mock mediator — same
// observation shape and before/after-send classification as the live witness
// proxy and the D2a VAPIX mock transport.
export class InMemoryMediatorWitnessTransport implements HttpTransport {
  constructor(readonly backend: MediatorBackend, readonly witnessLogPath: string) {}
